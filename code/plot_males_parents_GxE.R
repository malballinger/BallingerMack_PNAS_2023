...
# This script plots differential expression in male parental samples, with specific focus on patterns of GxE.
# This script generates Figure 2B in BallingerMack_2022.
...
liver_males_plasticity <- merge_plast_males_liver_direction %>%
  ...
  geom_point(data = sigNY_liver, aes(x = lfc.NY, y = lfc.BZ), fill =  "#1683B5", color = "grey100", stroke = 0.25, size = 2, shape = 21, alpha = 0.8, show.legend = TRUE) +
...
BAT_males_plasticity <- merge_plast_males_BAT_direction %>%
  ...
  geom_point(data = sigNY_BAT, aes(x = lfc.NY, y = lfc.BZ), fill = "#1683B5", color = "grey100", stroke = 0.25, size = 2, shape = 21, alpha = 0.8, show.legend = TRUE) +
