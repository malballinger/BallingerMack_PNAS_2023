...
# This script plots differential expression in parental samples, with specific focus on the comparison
# of the number of genes in each category, based on FDR cutoffs of 0.01, 0.05, 0.1.
# This script generates Table S2 in BallingerMack_2023.

# Major Result(s): overall patterns of divergence and GxE are similar, regardless of FDR cutoff.
...
# filter to get mean of 10 reads across all samples (per gene)
  DE_base_resW <- res_warm_males_liver_NYvsBZ %>%
    data.frame() %>%
    rownames_to_column(var = "gene") %>%
    as_tibble() %>%
    filter(baseMean >= 10)

  DE_base_resC <- res_cold_males_liver_NYvsBZ %>%
    data.frame() %>%
    rownames_to_column(var = "gene") %>%
    as_tibble() %>%
    filter(baseMean >= 10)
 ...
 # categorize genes by direction and/or significance
  sigC = dplyr::filter(merge_resWC, padj.c < 0.05 & padj.w >= 0.05)
  #sigsame = dplyr::filter(merge_resWC_direction, direction == 1 & padj.w < 0.05 & padj.c < 0.05)
  sigW = dplyr::filter(merge_resWC, padj.w < 0.05 & padj.c >= 0.05)
  #sigopps = dplyr::filter(merge_resWC_direction, direction == 0 & padj.w < 0.05 & padj.c < 0.05)
  nonsig = dplyr::filter(merge_resWC, padj.w >= 0.05 & padj.c >= 0.05)

  sigC_LFC_05 = dplyr::filter(merge_resWC, (padj.c < 0.05 & padj.w >= 0.05), (lfc.c > 0.5))
